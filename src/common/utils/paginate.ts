...
export const paginateResource = async (model, query) => {
    return await createPaginator({ perPage: query.perPage ? query.perPage : 20 })(model, {
    ...
    if (query.hasOwnProperty('sort')) {
        if (Array.isArray(query.sort)) {
            query.sort.forEach((value: string) => {
                sortQuery.push({ [value.replace('-', '')]: value.includes('-') ? 'desc' : 'asc' })
            })
        } else {
            sortQuery.push({ [query.sort.replace('-', '')]: query.sort.includes('-') ? 'desc' : 'asc' })
        }
    }
