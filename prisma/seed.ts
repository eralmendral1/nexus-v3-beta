...
import { policiesData, policyCategoryData, clientsData, contactsData, destinationNumberData, itemsData } from './data'
...
function seedItems() {
    itemsData.forEach(async (data) => {
        await prisma.item.upsert({ where: { id: data.id }, update: {},
             create: data })
    })
}
